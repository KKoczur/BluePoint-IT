...
public partial class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        {
            var columnToShow = logParser.GetLogPatterns().Select(pattern => Tuple.Create(pattern.KeyName, pattern.Show))
                .ToList();
        ...
        _logDirectoryPath = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData) +
                            _config.StatlookLogDirectory;
    ...
    {
        if (logParser == null) throw new ArgumentNullException(nameof(logParser));
        ...
        rootMenu.DropDownItemClicked += ToolStripMenuItemUplook_DropDownItemClicked;

        _toolStripMenuItemFilterColumnMap.Add(logParser.UniqueLogKey, rootMenu);
        viewToolStripMenuItem.DropDownItems.Add(rootMenu);

        rootMenu.DropDownItems.AddRange(logPatterns.Select(logPattern => new ToolStripMenuItem
        {
    ...
    private void SetFilesInfoGridViewData()
    {
        ...
        toolStripButtonIcon.Image = Resources.ok_16;
        toolStripStatusReady.Text = Resources.Ready;
    }
    ...
    private void OpenLogFile()
    {
        using var fileDialog = new OpenFileDialog
        {
            ...
            Title = Resources.SelectLogSourceTitle
        };
        ...
        if (fileDialog.FileNames.Length == 1 &&
            fileDialog.SafeFileName?.Substring(fileDialog.SafeFileName.Length - 4, 4) ==
            Configuration.ZIP_FILE_EXTENSION)
        {
    ...
    {
        if (MessageBox.Show(Resources.CloseApplicationQuestion, Resources.Close, MessageBoxButtons.YesNo,
    ...
    private void TabControlMain_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (selectedTabPage != tabPageInfo)
        {
            ...
            {
                MessageBox.Show(string.Format(Resources.FileDoesNotExist, fileInfo.FullName), Resources.Error,
    ...
    {
        return _tabControlMain.SelectedTab;
    }
    ...
    private void TabControlMain_MouseUp(object sender, MouseEventArgs e)
    {
        ...
        for (tabIndex = 0; tabIndex <= _tabControlMain.TabCount - 1; tabIndex++)
        {
            ...
            if (_tabControlMain.SelectedIndex != 0) contextMenuStripPage.Show(_tabControlMain, e.Location);
        }
    }
    ...
    private void ToolStripMenuItem7_Click(object sender, EventArgs e)
    {
        ...
        if (MessageBox.Show(string.Format(Resources.ConfirmDeletePrompt, fileInfo.FullName),
                Resources.ConfirmDeleteCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) !=
        ...
        {
            MessageBox.Show($@"{Resources.Error} : {exception.Message}", Resources.Error, MessageBoxButtons.OK,
    ...
    private void ToolStripMenuItemOpenFile_Click(object sender, EventArgs e)
    {
        ...
        if (listViewFiles.SelectedItems.Count == 1 && listViewFiles.SelectedItems[0].SubItems[1].Text
                .Substring(listViewFiles.SelectedItems[0].SubItems[1].Text.Length - 4, 4) == ".zip")
        {
    ...
    private void ToolStripMenuItemDeleteFile_Click(object sender, EventArgs e)
    {
        foreach (ListViewItem listViewItem in listViewFiles.SelectedItems)
        {
            ...
            if (MessageBox.Show(string.Format(Resources.ConfirmDeletePrompt, fileFullPath),
                    Resources.ConfirmDeleteCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) !=
                DialogResult.Yes)
                continue;
            ...
            {
                MessageBox.Show($@"{Resources.Error} : {ex.Message}", Resources.Error, MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        }
    }
    ...
    private void ToolStripMenuItemCopyFileName_Click(object sender, EventArgs e)
    {
        ...
        var filesNameText = fileCollection.Aggregate<FileInfo, string>(default,
            (current, fileInfo) => current + fileInfo.Name + Environment.NewLine);
    ...
    private void ToolStripMenuItemCopyFilePath_Click(object sender, EventArgs e)
    {
        ...
        var filesFullNameText = fileCollection.Aggregate<FileInfo, string>(default,
            (current, fileInfo) => current + fileInfo.FullName + Environment.NewLine);
    ...
    private void ToolStripMenuItemCopyCatalogPath_Click(object sender, EventArgs e)
    {
        ...
        var fileDirectoryNamesText = fileCollection.Aggregate<FileInfo, string>(default,
            (current, fileInfo) => current + fileInfo.DirectoryName + Environment.NewLine);
    ...
    {
        return (from ListViewItem listViewItem in listViewFiles.SelectedItems
                select GetFilePathFromListViewItem(listViewItem)
            into filePath
                select new FileInfo(filePath)).ToArray();
    }
    ...
    private void SetDashboardData(string directoryLogPath, Control label, Control labelSize, Control labelCount)
    {
        try
        {
            ...
            labelCount.Text = fileCollection.Length + " files, " + directoryCollection.Length + " directories";
        }
        catch
        {
            // ignored
        ...
        foreach (Control control in tabP.Controls)
        {
            ...
            if (toolStripTextBox.Text?.Length == 0)
            {
                ...
                try
                {
                    ...
                    {
                        listViewExtended.SetOneGroupState(listViewItem.Group,
                            ListViewGroupState.Collapsible | ListViewGroupState.Collapsed);
            ...
            {
                foreach (ListViewItem listViewItem in listViewExtended.Items) listViewItem.BackColor = Color.White;
