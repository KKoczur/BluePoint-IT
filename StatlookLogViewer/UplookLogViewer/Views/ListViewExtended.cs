...
public class ListViewExtended : ListView
{
    ...
    ///     If this parameter is HWND_BROADCAST, the message is sent to all top-level windows in the system, including disabled
    ///     or invisible unowned windows, overlapped windows, and pop-up windows; but the message is not sent to child windows.
    ///     Microsoft Windows Vista and later. Message sending is subject to User Interface Privilege Isolation (UIPI). The
    ...
    /// <returns>
    ///     Microsoft Windows Vista and later. When a message is blocked by UIPI the last error, retrieved with GetLastError,
    ///     is set to 5 (access denied).
    ///     Applications that need to communicate using HWND_BROADCAST should use the RegisterWindowMessage function to obtain
    ...
    ///     subroutine. If the specified window was created by a different thread, the system switches to that thread and calls
    ///     the appropriate window procedure. Messages sent between threads are processed only when the receiving thread
    ///     executes message retrieval code. The sending thread is blocked until the receiving thread processes the message.
    ///     However, the sending thread will process incoming nonqueued messages while waiting for its message to be processed.
    ///     To prevent this, use SendMessageTimeout with SMTO_BLOCK set. For more information on nonqueued messages, see
    ///     Nonqueued Messages.
    ///     Windows 95/98/Me: SendMessageW is supported by the Microsoft Layer for Unicode (MSLU). To use this, you must add
    ...
    private static void setGrpState(ListViewGroup lstvwgrp, ListViewGroupState state)
    {
        ...
        else
        {
            ...
            var gIndex = lstvwgrp.ListView.Groups.IndexOf(lstvwgrp);
...
public struct LVGROUP
{
    ...
    /// <summary>
    ///     Mask that specifies which members of the structure are valid input. One or more of the following values:LVGF_NONENo
    ///     other items are valid.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Pointer to a null-terminated string that contains the header text when item information is being set. If group
    ...
    /// <summary>
    ///     Size in TCHARs of the buffer pointed to by the pszHeader member. If the structure is not receiving information
    ...
    /// <summary>
    ///     Pointer to a null-terminated string that contains the footer text when item information is being set. If group
    ///     information is being retrieved, this member specifies the address of the buffer that receives the footer text.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Size in TCHARs of the buffer pointed to by the pszFooter member. If the structure is not receiving information
    ///     about a group, this member is ignored.
    /// </summary>
    [Description(
    ...
    ///     ID of the group.
    /// </summary>
    [Description("ID of the group.")] public int IGroupId;

    /// <summary>
    ///     Mask used with LVM_GETGROUPINFO (Microsoft Windows XP and Windows Vista) and LVM_SETGROUPINFO (Windows Vista only)
    ///     to specify which flags in the state value are being retrieved or set.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Flag that can have one of the following values:LVGS_NORMALGroups are expanded, the group name is displayed, and all
    ///     items in the group are displayed.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Indicates the alignment of the header or footer text for the group. It can have one or more of the following
    ///     values. Use one of the header flags. Footer flags are optional. Windows XP: Footer flags are
    ///     reserved.LVGA_FOOTER_CENTERReserved.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Pointer to a null-terminated string that contains the subtitle text when item information is being
    ///     set. If group information is being retrieved, this member specifies the address of the buffer that receives the
    ///     subtitle text. This element is drawn under the header text.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Size, in TCHARs, of the buffer pointed to by the pszSubtitle member. If the structure is not
    ///     receiving information about a group, this member is ignored.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Pointer to a null-terminated string that contains the text for a task link when item information is
    ///     being set. If group information is being retrieved, this member specifies the address of the buffer that receives
    ///     the task text. This item is drawn right-aligned opposite the header text. When clicked by the user, the task link
    ...
    /// <summary>
    ///     Windows Vista. Size in TCHARs of the buffer pointed to by the pszTask member. If the structure is not receiving
    ...
    /// <summary>
    ///     Windows Vista. Pointer to a null-terminated string that contains the top description text when item information is
    ///     being set. If group information is being retrieved, this member specifies the address of the buffer that receives
    ///     the top description text. This item is drawn opposite the title image when there is a title image, no extended
    ///     image, and uAlign==LVGA_HEADER_CENTER.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Size in TCHARs of the buffer pointed to by the pszDescriptionTop member. If the structure is not
    ///     receiving information about a group, this member is ignored.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Pointer to a null-terminated string that contains the bottom description text when item information
    ///     is being set. If group information is being retrieved, this member specifies the address of the buffer that
    ///     receives the bottom description text. This item is drawn under the top description text when there is a title
    ///     image, no extended image, and uAlign==LVGA_HEADER_CENTER.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. Size in TCHARs of the buffer pointed to by the pszDescriptionBottom member. If the structure is not
    ///     receiving information about a group, this member is ignored.
    /// </summary>
    [Description(
    ...
    /// <summary>
    ///     Windows Vista. NULL if group is not a subset. Pointer to a null-terminated string that contains the subset title
    ///     text when item information is being set. If group information is being retrieved, this member specifies the address
    ...
    /// <summary>
    ///     Windows Vista. Size in TCHARs of the buffer pointed to by the pszSubsetTitle member. If the structure is not
    ///     receiving information about a group, this member is ignored.
    /// </summary>
    [Description(
