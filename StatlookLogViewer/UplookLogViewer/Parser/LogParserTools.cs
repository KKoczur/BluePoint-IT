...
internal class LogParserTools
{
    public static Dictionary<string, ILogParser> GetLogParserMap()
    {
        ...
        return (from objectType in assembly.GetTypes()
                where objectType.GetInterfaces().Contains(typeof(ILogParser))
                select (ILogParser)Activator.CreateInstance(objectType)).ToDictionary(logParser => logParser.UniqueLogKey);
    }
    ...
    public static (ILogParser, string[]) DetectLogParser(string filePath)
    {
        ...
        foreach (var kvp in from kvp in GetLogParserMap()
                            from line in allFileLines
                            where line.Contains(kvp.Key)
                            select kvp)
        {
