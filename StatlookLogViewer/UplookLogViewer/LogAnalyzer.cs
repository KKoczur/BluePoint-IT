...
internal static class LogAnalyzer
{
    ...
    private static (List<ListViewGroup> groups, List<ListViewItem> listViewItems, ILogParser logParser) AnalyzeLogFile(
        string filePath)
    {
        ...
        foreach (var line in allFileLines)
            ...
            else if (line != null && !line.Contains(logParser.EndLogGroupEntry))
            {
                foreach (var textPattern in logParser.GetTextPatterns())
                {
                    ...
                    normalizeSingleLine =
                        normalizeSingleLine[..normalizeSingleLine.IndexOf(";", StringComparison.Ordinal)];
    ...
    private static void AnalyzeLine(ListViewItem listViewItem, string lineCaption, string lineValue,
        ILogParser logParser)
    {
        foreach (var logPattern in logParser.GetLogPatterns())
        {
            ...
            if (lineCaption != logParser.StartLogGroupEntry)
            {
                ...
                if (Regex.IsMatch(lineValue, @"(?<NR_1>\d{1})\.(?<NR_2>\d{1})\.(?<NR_3>\d{1})\b został uruchomiony.") ||
