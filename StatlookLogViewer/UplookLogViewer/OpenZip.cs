...
public partial class OpenZip : Form
{
    ...
    public OpenZip()
    {
        _zipDirectory = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData) + ZipTmpDirectory;
        InitializeComponent();
    ...
    private void CheckBoxSelectAll_Click(object sender, EventArgs e)
    {
        ...
        {
            foreach (ListViewItem item in listViewFiles.Items) item.Selected = false;
            listViewFiles.Focus();
        }
    }
    ...
    public void AddItem(ListViewItem listViewItem)
    {
        ...
        foreach (ColumnHeader ch in listViewFiles.Columns) ch.Width = -2;
    }
